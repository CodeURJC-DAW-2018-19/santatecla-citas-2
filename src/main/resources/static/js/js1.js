...
var appendQuote = '<div class="col-lg-6 col-md-6 col-sm-6 "> <div class="card card-stats"> <div class="card-header card-header-warning card-header-icon">' +
  '<div class="card-icon"> <i class="material-icons">speaker_notes</i> </div> <p class="card-category">Cita</p> <div class="tim-typo"> <blockquote class="blockquote">' +
  '{{#logged}}<a href="quote/{{id}}">{{/logged}} <p style="color: black">';
...
loadBtn.onclick = function () {
  ...
  $.ajax({
    ...
    for (var i = 0; i < quotes.length; i++) {
      var tpl = "" + appendQuote + quotes[i].text + appendQuote2 + quotes[i].author + ' ,' + quotes[i].book + appendQuote3 + "";
